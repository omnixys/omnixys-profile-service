...
export class ProfileReadService {
    ...
    async getMyFullProfile(username: string): Promise<FullProfile> {
        ...
        const followerCount = await this.#followReadService.getFollowerCount(profileId);
        const followingCount = await this.#followReadService.getFollowingCount(profileId);

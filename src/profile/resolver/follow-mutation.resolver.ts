...
export class FollowMutationResolver {
    ...
    @Mutation(() => Boolean)
    async followUser(@Args('followerId') followerId: string, @Args('followedId') followedId: string): Promise<boolean> {
        return this.#followWriteService.followUser(followerId, followedId);
    }

    @Mutation(() => Boolean)
    async unfollowUser(@Args('followerId') followerId: string, @Args('followedId') followedId: string): Promise<boolean> {
